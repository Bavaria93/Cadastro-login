...
function ListaUsuarios() {
  ...
  const fetchUsers = useCallback(() => {
    try {
      if (users.length === 0) {
        console.log("Nenhum usuário cadastrado até o momento.");
        return;
      }
  ...
  }, [users]); // Lista 'users' como dependência, mantendo estabilidade
  
  useEffect(() => {
    fetchUsers(); // Executa a função
  }, [fetchUsers]); // Inclui 'fetchUsers' como dependência estável
  ...
  const handleUpdateUser = () => {
    if (selectedUser) {
      try {
        // Simula a atualização de um usuário na lista local
        setUsers((prevUsers) =>
          prevUsers.map((user) =>
  ...
  const handleDeleteUser = () => {
    if (selectedUser) {
      try {
        // Simula a exclusão de um usuário na lista local
        setUsers((prevUsers) =>
